...
def placeorder(t, f_name, quantity):
    try:
        ...
        c.execute('SELECT COUNT(*) FROM bill WHERE f_id = (SELECT f_id FROM food_items WHERE f_name = %s) AND table_no = %s', (f_name, t))
        result = c.fetchone()

        if result[0] == 0:
            # The record does not exist, proceed with the INSERT
            c.execute('INSERT INTO bill (table_no, f_id, quantity) VALUES (%s, (SELECT f_id FROM food_items WHERE f_name = %s), %s)', (t, f_name, quantity))
            mydb.commit()
            ...
            c.execute('UPDATE bill SET quantity = quantity + %s WHERE f_id = (SELECT f_id FROM food_items WHERE f_name = %s) AND table_no = %s', (quantity, f_name, t))
            mydb.commit()
